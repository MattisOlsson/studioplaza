...
namespace StudioPlaza.Web.Templates.Web.UI.WebControls
{
	/// <summary>
	...
	public class Menu : WebControl, IContentTemplate
	{
		...
		{
			int maxIdx = ih.Children.Count - 1;
			int realIdx = 0;

			for (int idx = 0; idx < ih.Children.Count; idx++)
			{
				HierarchyNode<ContentItem> childHierarchy = ih.Children[idx];
				...
				string _cssClass = realIdx++ == maxIdx ? "last " : string.Empty;
				...
				{
					_cssClass = string.IsNullOrEmpty(_cssClass) ? "has-children" : _cssClass.TrimEnd() + " has-children";
					li.Attributes["class"] = _cssClass;
					li.Controls.Add(ul);
				}
			}
		}
